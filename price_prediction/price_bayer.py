...
def do_main(stock_name, config):
    ...
    if test_method == 'split':
        ...
        test_day = config['test_days']
        df_train = df[:-test_day]
        df_test = df[-test_day:]
    parameter = load_hyper_parameter(stock_name, config)
    if parameter is None:
        config['train_verbose'] = False
        config['save_model'] = False
        old_epoch = config['epochs']
        config['epochs'] = 150
        tune_hyper_parameter(stock_name, df.copy(), config)
        parameter = load_hyper_parameter(stock_name, config)
        config['epochs'] = old_epoch
    ...
    if train_result is None or force_train:
        config['train_verbose'] = 1
        train_result = do_train(stock_name, df_train.copy(), config)
        test_result = do_test(stock_name, df_test.copy(), train_result ,config)
        plot_test_result(test_result['df'], stock_name, config)
        do_train_untrain(stock_name, df_test.copy(), train_result ,config)
    else:
